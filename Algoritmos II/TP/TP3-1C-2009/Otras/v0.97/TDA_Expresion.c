...
@see -
*/
void AStringInOrden (TAB ab,int MOV,char **string,int *size,int *reallocunavez)
{
    TLiteral eab;
    int error;
    ...
    if ( !error )
    {
        ...
        AStringInOrden(ab,IZQ,string,size,reallocunavez);

        AB_ElemCte(ab,&eab);
        *size = (*size) + 3;
        *string=(char*)realloc(*string,sizeof(char)*(*size));
        if (!(*reallocunavez)){
            strcpy(*string,eab.valor);
            *reallocunavez = 1;
        }
        else strcat(*string,eab.valor);


        AStringInOrden(ab,DER,string,size,reallocunavez);
...
int Expresion_AString(const TExpresion* expr, char** string)
{
    ...
    */
    ...
    AStringInOrden (expr->expresion,RAIZ,string,&size,&reallocunavez);
    /* printf ("\n\n\n"); */
    return 0;
}

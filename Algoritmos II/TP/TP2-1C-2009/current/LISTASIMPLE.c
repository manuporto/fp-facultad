...
void ls_BorrarCorriente(TListaSimple *pLs){
    ...
    if (pLs->Corriente==pLs->Primero){
        ...
        while (PAux->Siguiente!=pLs->Corriente) PAux = PAux->Siguiente;
...
int ls_Insertar(TListaSimple *pLs, TMovimiento_Ls M, void* pE){
    ...
    if (!pNodo) return FALSE; /*No hay memoria disponible */
    if ((pLs->Primero == NULL) || (M==LS_PRIMERO) || ((M==LS_ANTERIOR) && (pLs->Primero==pLs->Corriente))){
    /*Si está vacía o hay que insertar en el primero o hay que insertar en el anterior y el actual es el primero */

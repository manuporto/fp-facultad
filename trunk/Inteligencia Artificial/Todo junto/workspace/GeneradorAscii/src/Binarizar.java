...
public class Binarizar {
     ...
     static final int DIMENSION_FINAL = 80;
     static final int TAMANIO_MAXIMO = 80;
	...
	public int GenerarMatrizBinaria(String ImagenMono) throws IOException {
	     ...
	     while (buffImagen.getHeight() > fila){
	    	 ...
	    	 while (buffImagen.getWidth() > columna){
	    		 if (buffImagen.getRGB(columna, fila) < -70000){
...
//				System.out.println("La primera columna es "+i+"\n");
		...
		while (j != 0 && i<this.ancho){
			...
			if (j == 0){
				ultimaColumna = i;
//				System.out.println("La última columna es "+i+"\n");
			}
			i++;
		}
...
//		    imprimir (matrizAuxFila);
		    ...
		    System.out.println("["+numeroDeNumero+"]Ancho = "+matriz.length);
		    System.out.println("["+numeroDeNumero+"]Alto = "+(hasta-desde));
		    System.out.println();
